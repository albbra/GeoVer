...
CograBinaryMeshFile::CograBinaryMeshFile(const CograBinaryMeshFile& other)
{
  ...
  for(size_t i = 0; i < other.m_attributes.size(); i++)
  {
    ...
    m_attributes[i] = new uint8[nBytes];
  ...
  for(SizeType i = 0; i < getNumConstants(); i++)
  {
    ...
    m_constants[i] = new uint8[nBytes];
...
CograBinaryMeshFile::CograBinaryMeshFile(CograBinaryMeshFile && other)
{ 
  m_positions = std::move(other.m_positions);
  m_triangles = std::move(other.m_triangles);
  m_attributes = std::move(other.m_attributes);
  m_attributeComponents = std::move(other.m_attributeComponents);
  m_attributeComponentSize = std::move(other.m_attributeComponentSize);
  m_attributeNames = std::move(other.m_attributeNames);
  m_constants = std::move(other.m_constants);
  m_constantComponents = std::move(other.m_constantComponents);
  m_constantComponentSize = std::move(other.m_constantComponentSize);
  m_constantNames = std::move(other.m_constantNames);
}
...
void CograBinaryMeshFile::load(const std::string& fileName)
{
  ...
  if(!inFile.is_open())
  {
    throw RuntimeError("Error opening file" + normalizeFilename(fileName) + ".");
  }
...
{
  return static_cast<uint32>(m_positions.size() / 3);
}

CograBinaryMeshFile::SizeType CograBinaryMeshFile::getNumTriangles() const
{
  return static_cast<uint32>(m_triangles.size() / 3);
}
...
void CograBinaryMeshFile::readHeader(std::ifstream& inFile)
{
  ...
  if(nA != 0)
  {
    ...
    for(SizeType i = 0; i < getNumAttributes(); i++)
    {
      m_attributes[i] = new uint8[m_attributeComponents[i] * m_attributeComponentSize[i] * getNumVertices()];
      m_attributeNames[i] = new char[N_CHARS];
  ...
  if(nC != 0)
  {
    ...
    for(SizeType i = 0; i < getNumConstants(); i++)
    {
      m_constants[i] = (new uint8[m_constantComponents[i] * m_constantComponentSize[i]]);
      m_constantNames[i] = new char[N_CHARS];
      inFile.read((char*)m_constantNames[i], sizeof(uint8) * N_CHARS);
    }
  }
}
...
bool CograBinaryMeshFile::add(const CograBinaryMeshFile& src)
{
  ...
  memcpy((void*)&vertices[this->getNumVertices() * 3], (void*)src.getPositionsPtr(), src.getNumVertices() * 3 * sizeof(FloatType));
  ...
  std::vector<uint8*> attributes(nAttributes);
  for(SizeType i = 0; i < nAttributes; i++)
  {
    SizeType attribSize = src.getAttributeComponents(i) * src.getAttributeComponentSize(i);
    auto attr = new uint8[(this->getNumVertices() + src.getNumVertices()) * attribSize];
    memcpy((void*)&attr[0], this->getAttributePtr(i), this->getNumVertices() * attribSize);
    memcpy((void*)&attr[this->getNumVertices() * attribSize], src.getAttributePtr(i), src.getNumVertices() * attribSize);
    attributes[i] = attr;
  }
...
{
  return static_cast<uint32>(m_attributeComponents.size());
}

CograBinaryMeshFile::SizeType CograBinaryMeshFile::addAttribute(const void* attribute, const SizeType nComponents, const SizeType componentSize, const std::string& attributeName)
{
  ...
  SizeType l = std::min<SizeType>(static_cast<uint32>(N_CHARS), static_cast<uint32>(attributeName.length()));
  ...
  return static_cast<uint32>(m_attributes.size());
}
...
{
  return static_cast<uint32>(m_constantComponentSize.size());
}
...
CograBinaryMeshFile::SizeType CograBinaryMeshFile::addConstant(
  const void* constant,
  const SizeType nComponents,
  const SizeType componentSize, const std::string& constantName)
{
...
void CograBinaryMeshFile::overwriteConstants(const CograBinaryMeshFile& src)
{
  ...
  {
    addConstant(src.getConstant(t), src.getConstantComponents(t), src.getConstantComponentSize(t), src.getConstantName(t));
  }
}

int CograBinaryMeshFile::getConstantIdx(const char* name) const
{
  for(uint32 i = 0; i < getNumConstants(); i++)
...
{
  // Add the vertices
  ((float32 * const)result)[vIdx + 0] = m_positions[vIdx + 0];
  ((float32 * const)result)[vIdx + 1] = m_positions[vIdx + 1];
  ((float32 * const)result)[vIdx + 2] = m_positions[vIdx + 2];
  SizeType offset = 3 * 4;
  // Add the attributes.
  for(SizeType aIdx = 0; aIdx < getNumAttributes(); aIdx++)
    for(SizeType cIdx = 0; cIdx < getAttributeElementSize(aIdx); cIdx++)
    {
      ((unsigned char * const)result)[offset++] = ((unsigned char const*)getAttributePtr(aIdx))[vIdx * getAttributeElementSize(aIdx) + cIdx];
    }
}
...
void CograBinaryMeshFile::printConstant(std::ostream& stream) const
{
  ...
  for(SizeType t = 0; t < getNumConstants(); t++)
  {
    stream << t
      ...
      << "\t" << getConstantComponentSize(t)
      << "\t" << getConstantComponents(t)
      << "\t" << getConstantName(t) << "\n";
  }
  stream << "\n";
}
